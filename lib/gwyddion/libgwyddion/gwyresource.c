/*
 *  $Id: gwyresource.c 25036 2022-09-22 12:24:13Z yeti-dn $
...
static void         gwy_resource_class_load_dir (const gchar *path,
                                                 ...
                                                 gboolean is_system);
static void         save_resource_if_modified   (gpointer key,
                                                 gpointer value,
                                                 gpointer user_data);
static gboolean     gwy_resource_save_real      (GwyResource *resource,
                                                 GError **error);
...
gwy_resource_class_init(GwyResourceClass *klass)
{
    ...
    pspec = g_param_spec_string("name", "Name", "Resource name", NULL,
    ...
    pspec = g_param_spec_boolean("is-preferred", "Is preferred", "Whether a resource is preferred", FALSE,
                                 G_PARAM_READWRITE);
    ...
    pspec = g_param_spec_boolean("is-const", "Is constant", "Whether a resource is constant (system)", FALSE,
                                 G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
     ...
     */
    resource_signals[DATA_CHANGED] = g_signal_new("data-changed",
                          ...
                          GParamSpec *pspec)
{
    ...
    switch (prop_id) {
        case PROP_NAME:
        /* Only happens upon construction, and usually we just get NULL. GLib interprets G_PARAM_CONSTRUCT_ONLY as
         * *must* be set upon construction, not *may* be only set upon construction. */
        if (g_value_get_string(value))
            g_string_assign(resource->name, g_value_get_string(value));
        break;
 ...
 *
 * Call to this function is necessary to use a resource properly. It makes the resource to create any auxiliary
 * structures that consume considerable amount of memory and perform other initialization to ready-to-use form.
 *
 ...
 * use this method is the constructor and gwy_resource_release() is the destructor.
 **/
void
gwy_resource_use(GwyResource *resource)
{
    g_return_if_fail(GWY_IS_RESOURCE(resource));
    gwy_debug("%s %p<%s> %d", G_OBJECT_TYPE_NAME(resource), resource, resource->name->str, resource->use_count);
 ...
 * addition, it calls g_object_unref() on it.  See gwy_resource_use() for more.
 **/
void
 ...
 * @expected_type: Resource object type.  If not 0, only resources of give type are allowed.  Zero value means any
 *                 #GwyResource is allowed.
 *
                        ...
                        gboolean is_const)
{
    ...
    if (klass->parse_with_type)
        resource = klass->parse_with_type(type, text, is_const);
    else if (klass->parse)
                            ...
                            gboolean is_system)
{
    ...
    while ((name = g_dir_read_name(dir))) {
        ...
        resource = gwy_resource_parse_real(text, G_TYPE_FROM_CLASS(klass), is_system);
        if (resource) {
            g_string_assign(resource->name, name);
            resource->is_modified = FALSE;
...
gwy_resource_class_save_modified(GwyResourceClass *klass)
{
    ...
    gwy_inventory_foreach(klass->inventory, save_resource_if_modified, NULL);
}

static void
save_resource_if_modified(gpointer key, gpointer value, G_GNUC_UNUSED gpointer user_data)
{
    G_GNUC_UNUSED guint i = GPOINTER_TO_UINT(key);
    GwyResource *resource = (GwyResource*)value;

    if (resource->is_modified && !resource->is_const)
        gwy_resource_save_real(resource, NULL);
}
...
gwy_resource_build_filename(GwyResource *resource)
{
    ...
    g_return_val_if_fail(GWY_IS_RESOURCE(resource), NULL);
    if (resource->is_const)
        g_warning("Filename of a constant resource `%s' should not be needed", resource->name->str);

    klass = GWY_RESOURCE_GET_CLASS(resource);
    return g_build_filename(gwy_get_user_dir(), klass->name, resource->name->str, NULL);
}
 ...
 *
 * Only non-constant resources can be saved. The file name is determined by gwy_resource_build_filename(). The
                  ...
                  GError **error)
{
    ...
    if (resource->is_const) {
        g_warning("Constant resource is being passed to gwy_resource_save()");
        return TRUE;
    }
    ...
    return gwy_resource_save_real(resource, error);
}

static gboolean
gwy_resource_save_real(GwyResource *resource,
                       GError **error)
{
    gchar *filename;
    gboolean ok;
 ...
 * name (gwy_resource_get_name()), modifiability (gwy_resource_get_is_modifiable()), loading resources from files and
 * saving them.
 **/
 ...
 * @dump: gwy_resource_dump() virtual method. It only cares about resource data itself, the envelope is handled by
 *        #GwyResource.
 * @parse: gwy_resource_parse() virtual method. It only cares about resource data itself, the envelope is handled by
 *         #GwyResource. This is the usual method as the parse normally knows its type.
 * @parse_with_type: gwy_resource_parse() virtual method. It only cares about resource data itself, the envelope is
 *                   handled by #GwyResource. It takes precedence over @parse if both are defined. (Since 2.62)
 *
 * Resource class.
 **/
