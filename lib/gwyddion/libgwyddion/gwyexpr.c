/*
 *  $Id: gwyexpr.c 22340 2019-07-25 10:23:59Z yeti-dn $
...
struct _GwyExpr {
    ...
    guint level;   /* level in transform to rpn */
};
...
#endif
#if HAVE_TGAMMA
make_function_1_1(tgamma);
#endif
#if HAVE_J0
make_function_1_1(j0);
#endif
#if HAVE_J1
make_function_1_1(j1);
#endif
#if HAVE_Y0
make_function_1_1(y0);
#endif
#if HAVE_Y1
make_function_1_1(y1);
#endif
...
#else
static void gwy_expr_exp2(gdouble **s) { **s = exp(G_LN2 * **s); }
#endif

#if HAVE_LOG2
make_function_1_1(log2)
#else
static void gwy_expr_log2(gdouble **s) { **s = log(**s)/G_LN2; }
#endif
...
static const GwyExprFunction call_table[] = {
    ...
    { gwy_expr_gwy_spow,   "spow",   2,  1,  GWY_EXPR_CODE_SPOW,     },
    ...
    { gwy_expr_gwy_sinc,   "sinc",   1,  1,  GWY_EXPR_CODE_SINC,     },
...
#endif
#if HAVE_TGAMMA
    { gwy_expr_tgamma,     "Gamma",  1,  1,  GWY_EXPR_CODE_GAMMA,    },
#endif
#if HAVE_J0
    { gwy_expr_j0,         "J0",     1,  1,  GWY_EXPR_CODE_J0,       },
#endif
#if HAVE_J1
    { gwy_expr_j1,         "J1",     1,  1,  GWY_EXPR_CODE_J1,       },
#endif
#if HAVE_Y0
    { gwy_expr_y0,         "Y0",     1,  1,  GWY_EXPR_CODE_Y0,       },
#endif
#if HAVE_Y1
    { gwy_expr_y1,         "Y1",     1,  1,  GWY_EXPR_CODE_Y1,       },
#endif
};
...
{
    return GUINT_TO_POINTER(g_quark_from_static_string("gwy-expr-error-quark"));
}
    ...
    for (i = 0; i < expr->in; i++) {
        ...
        if (code->type == GWY_EXPR_CODE_CONSTANT)
            *(++sp) = code->value;
            ...
            call_table[code->type].function(&sp);
        else
            *(++sp) = expr->variables[-(gint)code->type];
    }
}
...
gwy_expr_stack_interpret_vectors(GwyExpr *expr,
                                 ...
                                 gdouble *result)
{
    /* XXX: We cannot have default(none) here due to incompatibility in OpenMP
     * specification.  Some versions then require shared(call_table), whereas
     * some forbid it. */
#ifdef _OPENMP
#pragma omp parallel if(gwy_threads_are_enabled()) \
...
#endif
    {
        ...
        guint jfrom = gwy_omp_chunk_start(n), jto = gwy_omp_chunk_end(n);
        guint i, j, nin = expr->in;
        const GwyExprCode *input = expr->input;

        for (j = jfrom; j < jto; j++) {
            gdouble *sp = stack - 1;
            for (i = 0; i < nin; i++) {
                const GwyExprCode *code = input + i;
                ...
                if (code->type == GWY_EXPR_CODE_CONSTANT)
                    ...
                    *(++sp) = data[-(gint)code->type][j];
            }
            result[j] = stack[0];
        }
        gwy_omp_if_threads_free(expr->stack, stack);
    }
}
...
static gpointer
ensure_call_table_sanity(G_GNUC_UNUSED gpointer arg)
{
    if (!gwy_expr_check_call_table_sanity())
        return GUINT_TO_POINTER(G_MAXUINT);
...
gwy_expr_new(void)
{
    static GOnce call_table_sanity_once = G_ONCE_INIT;
    GwyExpr *expr;

    g_once(&call_table_sanity_once, ensure_call_table_sanity, NULL);
    /* Very bad things will happen and we cannot stop them... */
    if (GPOINTER_TO_UINT(call_table_sanity_once.retval) == G_MAXUINT)
        return NULL;
