/*
 * $Id: gwymd5.c 20678 2017-12-18 18:26:55Z yeti-dn $
 *
...
typedef struct _GwyMD5Context GwyMD5Context;

struct _GwyMD5Context {
    guint32 buf[4];
    guint32 bits[2];
    guchar  in[64];
};
...
#  define byte_reverse(b, l) /* noop */
#else
#  define byte_reverse(b, l) byte_reverse_real((b), (l))
#endif

static void gwy_md5_init      (GwyMD5Context *ctx);
static void gwy_md5_transform (guint32 buf[4],
                               const guint32 in[16]);
static void gwy_md5_update    (GwyMD5Context *ctx,
                               const guchar *buf,
                               guint32 len);
static void gwy_md5_final     (GwyMD5Context *ctx,
                               guchar digest[16]);
                   ...
                   guchar digest[16])
{
    ...
    gwy_md5_init(&ctx);
    gwy_md5_update(&ctx, buffer, buffer_size);
    gwy_md5_final(&ctx, digest);
}

#if (G_BYTE_ORDER != G_LITTLE_ENDIAN)
static inline void
byte_reverse_real(guint32 *buf,
                  guint32 longs)
{
    do {
...
static void
gwy_md5_init(GwyMD5Context *ctx)
{
...
static void
gwy_md5_update(GwyMD5Context *ctx,
               const guchar *buf,
               guint32 len)
{
    guint32 t;
...
static void
gwy_md5_final(GwyMD5Context *ctx,
              guchar digest[16])
{
    guint32 count;
    guchar *p;
 ...
 */
static void
gwy_md5_transform(guint32 buf[4], const guint32 in[16])
{
