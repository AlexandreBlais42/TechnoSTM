/*
 *  $Id: gwygrainvalue.c 23204 2021-03-08 15:31:59Z yeti-dn $
 *  Copyright (C) 2007-2018 David Necas (Yeti).
...
grain_values[] = {
    ...
    {
        N_("Minimum Martin diameter"),
        GWY_GRAIN_VALUE_MINIMUM_MARTIN_DIAMETER,
        {
            GWY_GRAIN_VALUE_GROUP_BOUNDARY,
            "<i>M</i><sub>min</sub>", "M_min", NULL,
            1, 0, 0, 0,
        }
    },
    {
        N_("Direction of minimum Martin diameter"),
        GWY_GRAIN_VALUE_MINIMUM_MARTIN_ANGLE,
        {
            GWY_GRAIN_VALUE_GROUP_BOUNDARY,
            "<i>ω</i><sub>min</sub>", "omega_min", NULL,
            0, 0, GWY_GRAIN_VALUE_IS_ANGLE, 0,
        }
    },
    {
        N_("Maximum Martin diameter"),
        GWY_GRAIN_VALUE_MAXIMUM_MARTIN_DIAMETER,
        {
            GWY_GRAIN_VALUE_GROUP_BOUNDARY,
            "<i>M</i><sub>max</sub>", "M_max", NULL,
            1, 0, 0, 0,
        }
    },
    {
        N_("Direction of maximum Martin diameter"),
        GWY_GRAIN_VALUE_MAXIMUM_MARTIN_ANGLE,
        {
            GWY_GRAIN_VALUE_GROUP_BOUNDARY,
            "<i>ω</i><sub>max</sub>", "omega_max", NULL,
            0, 0, GWY_GRAIN_VALUE_IS_ANGLE, 0,
        }
    },
    {
...
gwy_grain_value_data_sanitize(GwyGrainValueData *data)
{
    ...
    if (!g_utf8_validate(data->symbol_markup, -1, NULL)) {
        ...
        gwy_assign_string(&data->symbol_markup, data->symbol);
    }
                      ...
                      gboolean is_const)
{
    ...
    while ((line = gwy_str_next_line(&p))) {
        ...
        if (gwy_strequal(key, "symbol") && strlen(value))
            gwy_assign_string(&data.symbol, value);
        else if (gwy_strequal(key, "symbol_markup") && strlen(value))
            gwy_assign_string(&data.symbol_markup, value);
                                  ...
                                  const gchar *symbol)
{
    ...
    gwy_assign_string(&gvalue->data.symbol_markup, symbol);
                           ...
                           const gchar *symbol)
{
    ...
    gwy_assign_string(&gvalue->data.symbol, symbol);
    gwy_resource_data_changed(GWY_RESOURCE(gvalue));
}
                               ...
                               GError **error)
{
    ...
    if (!gwy_grain_value_resolve_expression(expression, NULL, NULL, &err)) {
        if (err)
    ...
    gwy_assign_string(&gvalue->expression, expression);
    gwy_resource_data_changed(GWY_RESOURCE(gvalue));
                           ...
                           const gint *grains)
{
    ...
    for (i = 0; i < nvalues; i++) {
        ...
        if (gvalue->data.group != GWY_GRAIN_VALUE_GROUP_USER)
            continue;
        gwy_grain_value_resolve_expression(gvalue->expression, vars, expr,
                                           NULL);
        gwy_clear(mapped, MAXBUILTINS + 1);
        for (q = 0; q < MAXBUILTINS; q++) {
            if (vars[q]) {
