/*
 *  $Id: spline.c 23812 2021-06-08 12:35:55Z yeti-dn $
...
gwy_spline_length(GwySpline *spline)
{
    ...
    if (G_UNLIKELY(!tl_points)) {
        spline->tl_points = g_array_sized_new(FALSE, FALSE, sizeof(GwyXY),
    ...
    if (cparam->depth == MAX_RECURSION_DEPTH
        ...
        switch (cparam->otype) {
            ...
            case CURVE_RECURSE_OUTPUT_T_L:
            {
                GwyXY tl;
                tl.x = c1->t;
                tl.y = (myhypot(c0->z.x - cc.z.x, c0->z.y - cc.z.y)
                        + myhypot(cc.z.x - c1->z.x, cc.z.y - c1->z.y));
                       ...
                       GwySplineRecurseOutputType otype)
{
    ...
    if (G_UNLIKELY(!tangents)) {
        spline->tangents = g_array_sized_new(FALSE, FALSE, sizeof(GwyXY),
                                             points->len);
