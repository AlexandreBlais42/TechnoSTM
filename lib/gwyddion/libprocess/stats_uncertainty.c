/*
 *  $Id: stats_uncertainty.c 24847 2022-06-28 10:25:16Z yeti-dn $
 ...
 *
 * Note that this is not the uncertainty arising from the calculation of the
 * maximum.  It is just the uncertainty of the single value that happens to be
 * the data maximum.  As such, the function is not very useful.
 *
    ...
    if (mask) {
        ...
        for (i = 0; i < height; i++) {
            ...
            if (mode == GWY_MASK_INCLUDE) {
                ...
                for (j = 0; j < width; j++) {
                    if (G_UNLIKELY(max < *drow) && *mrow < 1.0) {
                        max = *drow;
                        max_unc = *urow;
                    }
                    drow++;
 ...
 *
 * Note that this is not the uncertainty arising from the calculation of the
 * minimum.  It is just the uncertainty of the single value that happens to be
 * the data minimum.  As such, the function is not very useful.
 *
    ...
    if (mask) {
        ...
        for (i = 0; i < height; i++) {
            ...
            if (mode == GWY_MASK_INCLUDE) {
                ...
                for (j = 0; j < width; j++) {
                    if (G_UNLIKELY(min > *drow) && *mrow < 1.0) {
                        min = *drow;
                        min_unc = *urow;
                    }
                    drow++;
 ...
 *
 * Note that this is not the uncertainty arising from the calculation of the
 * minimum and maximum.  It is just the uncertainty of the two values that
 * happen to be the data minimum and maximum.  As such, the function is not
 * very useful.
 *
 * Since: 2.23
 **/
                                       ...
                                       gdouble *max_unc)
{
    ...
    min_unc1 = uncz_field->data[0];
    max_unc1 = uncz_field->data[0];
    p = data_field->data;
    u = uncz_field->data;
    for (i = data_field->xres * data_field->yres; i; i--, p++, u++) {
                                                 ...
                                                 gdouble *max_unc)
{
    ...
    /* Static code analysis: sod off.  We ensure just above that at most one
     * pointer is NULL. */
    if (!min_unc) {
        *max_unc = gwy_data_field_area_get_max_uncertainty(data_field, uncz_field, NULL,
                GWY_MASK_IGNORE, col, row, width, height);
        return;
    }
    if (!max_unc) {
                                             ...
                                             gint width, gint height)
{
    ...
    if (!_gwy_data_field_check_area(dfield, col, row, width, height)
        || !_gwy_data_field_check_mask(dfield, &mask, &mode))
        return 0.0;
    g_return_val_if_fail(GWY_IS_DATA_FIELD(uncz_field), 0.0);
...
typedef void (*GwyLameAreaUFunc)(GwyDataLine *source,
                                 GwyDataLine *usource,
                                 GwyDataLine *target);

static void
gwy_data_field_area_func_lame_uncertainty(GwyDataField *data_field,
                                          GwyDataField *uncz_field,
...
*/
    for (m = 0; m < n; m++) {
        if (m > (n-1)/2.0) {
            ...
            for (i = m; i < n-m; i++) {
                val = 2*data_line->data[i] - data_line->data[i-m] - data_line->data[i+m];
                target_line->data[m] += val*val*uline->data[i]*uline->data[i];
            }
            for (i = n-m; i < n; i++) {
                val = data_line->data[i] - data_line->data[i-m];
                         ...
                         gdouble q)
{
    ...
    a23 = sqrt(1.0 + 2.0*(z2*z2 + z3*z3)/q);
    a34 = sqrt(1.0 + 2.0*(z3*z3 + z4*z4)/q);
                                                      ...
                                                      gint width, gint height)
{
    ...
    if (!_gwy_data_field_check_area(data_field, col, row, width, height)
        || !_gwy_data_field_check_mask(data_field, &mask, &mode))
        return uarea;
    g_return_val_if_fail(GWY_IS_DATA_FIELD(uncz_field), uarea);
 ...
 *
 * Note that this is not the uncertainty arising from the calculation of the
 * median.  It is just the uncertainty of the single value that happens to be
 * the data median.  As such, the function is not very useful.
 *
...
gwy_data_field_area_get_median_uncertainty(GwyDataField *dfield,
                                           ...
                                           gint width, gint height)
{
    return gwy_data_field_area_get_median_uncertainty_mask(dfield, uncz_field,
                                                           mask,
                                                           GWY_MASK_INCLUDE,
                                                           col, row,
                                                           width, height);
                                                ...
                                                gint width, gint height)
{
    ...
    if (!_gwy_data_field_check_area(dfield, col, row, width, height))
        return med_unc;
    g_return_val_if_fail(GWY_IS_DATA_FIELD(uncz_field), med_unc);
                                   ...
                                   gint nstats)
{
    ...
    /* Find minimum and maximum and their position */
    min = G_MAXDOUBLE;
    max = -G_MAXDOUBLE;

    if (mask) {
        for (j = 0; j < height; j++) {
            drow = data_field->data + (j + row)*data_field->xres + col;
            mrow = mask->data + (j + row)*mask->xres + col;
    ...
    /* Handle border cases */
    if (min == max) {
        max_unc = gwy_data_field_get_val(uncz_field, imax, jmax);
    ...
    if (mask) {
        /* all terms except maximum and minimum */
        for (l = 0; l < height; l++) {
            urow = uncz_field->data + (l + row)*data_field->xres + col;
            drow = data_field->data + (l + row)*data_field->xres + col;
            mrow = mask->data + (l + row)*mask->xres + col;
            for (k = 0; k< width; k++) {
                if (mrow[k]) {
                    m = (gint)((drow[k] - min)/(max - min)*nstats);
                    /* Fix rounding errors */
                    if (G_UNLIKELY(m >= nstats))
                        m = nstats-1;
                    else if (G_UNLIKELY(m < 0))
                        m = 0;
                    target_line->data[m]+=nstats*nstats*urow[k]*urow[k];
                }
            }
        }

        /* contribution from maximum and minimum */
        valmin = g_new0(gdouble,nstats); 
        valmax = g_new0(gdouble,nstats); 

        for (j = 0; j < height; j++) {
            drow = data_field->data + (j + row)*data_field->xres + col;
            urow = uncz_field->data + (j + row)*data_field->xres + col;
            urow = mask->data + (j + row)*data_field->xres + col;
            mrow = mask->data + (l + row)*mask->xres + col;
            for (i = 0; i < width; i++) {
                if (mrow[i]) {
                    m = (gint)((drow[i] - min)/(max - min)*nstats);
                    /* Fix rounding errors */
                    if (G_UNLIKELY(m >= nstats))
                        m = nstats-1;
                    else if (G_UNLIKELY(m < 0))
                        m = 0;
                    valmin[m] += (drow[i] - max)/(max - min)*nstats + 1;
                    valmax[m] -= (drow[i] - min)/(max - min)*nstats + 1;
                }
            }
        }

        for (m = 0; m < nstats; m++) {
            target_line->data[m]+=valmin[m]*valmin[m]*gwy_data_field_get_val(uncz_field, imin, jmin)*gwy_data_field_get_val(uncz_field, imin, jmin);
            target_line->data[m]+=valmax[m]*valmax[m]*gwy_data_field_get_val(uncz_field, imax, jmax)*gwy_data_field_get_val(uncz_field, imax, jmax);
        }

        m = (gint)((gwy_data_field_get_val(data_field,imin,jmin) - min)/(max - min)*nstats);
        /* Fix rounding errors */
        if (G_UNLIKELY(m >= nstats))
            m = nstats-1;
        else if (G_UNLIKELY(m < 0))
            m = 0;
        target_line->data[m]+=2*nstats*valmin[m]*gwy_data_field_get_val(uncz_field, imin, jmin)*gwy_data_field_get_val(uncz_field, imin, jmin);

        m = (gint)((gwy_data_field_get_val(data_field,imax,jmax) - min)/(max - min)*nstats);
        /* Fix rounding errors */
        if (G_UNLIKELY(m >= nstats))
            m = nstats-1;
        else if (G_UNLIKELY(m < 0))
            m = 0;
        target_line->data[m]+=2*nstats*valmax[m]*gwy_data_field_get_val(uncz_field, imax, jmax)*gwy_data_field_get_val(uncz_field, imax, jmax);
    ...
    for (m = 0; m < nstats; m++)
        target_line->data[m]=sqrt(target_line->data[m]);

    gwy_data_line_multiply(target_line, nstats/((max - min)*(max - min)*nn));
...
gwy_data_field_get_xder_uncertainty(GwyDataField *data_field,
    ...
    hx = gwy_data_field_get_dx(data_field);
    hy = gwy_data_field_get_dy(data_field);
