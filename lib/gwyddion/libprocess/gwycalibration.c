/*
 *  $Id: gwycalibration.c 25360 2023-05-25 12:11:02Z yeti-dn $
...
static void
gwy_calibration_init(G_GNUC_UNUSED GwyCalibration *calibration)
{
}
                    ...
                    const gchar *filename)
{
    ...
    if (filename)
        calibration->filename = g_strdup(filename);
                     ...
                     GString *str)
{
    ...
    g_string_append_printf(str, "npoints %u\nfilename %s\n",
                           gwy_calibration_get_ndata(calibration),
                           calibration->filename);
}
