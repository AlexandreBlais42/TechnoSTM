...
void Image::initialize(const uint32_t XRes, const uint32_t YRes,
                       const std::string title, const std::string ZUnits) {
  this->XRes = XRes;
  this->YRes = YRes;
  this->title = title;
  this->ZUnits = ZUnits;
  ...
  data = std::vector<std::vector<float>>(
      XRes, std::vector<float>(YRes)); // Empty Vector
}

void Image::setPixel(const uint32_t XCoordinate, const uint32_t YCoordinate,
                     const float value) {
...
std::vector<uint8_t> Image::getGSFFile() {
  ...
  // Réserver assez d'espace dans le vecteur pour ne pas devoir en réserver dans
  // chaque push_back
  GSFFile.reserve(XRes * YRes * 4 + 1024);

  // Écrits le header bien formatté
  std::string header = std::string("Gwyddion Simple Field 1.0") +
